...
/* ------------------------------------ */
	private function _create_tabs($tabs,$count) {
		...
		$titles = array(
			'recent'	=> esc_html__('Recent Posts','enspire'),
			'popular'	=> esc_html__('Popular Posts','enspire'),
			'comments'	=> esc_html__('Recent Comments','enspire'),
			'tags'		=> esc_html__('Tags','enspire')
		);
		$icons = array(
			...
			<ul id="tab-comments" class="alx-tab group <?php if($instance['comments_avatars']) { echo 'avatars-enabled'; } ?>">
				...
				<li>
						...
						<div class="tab-item-inner group">
							...
							<div class="tab-item-name"><?php echo esc_attr( $comment->comment_author ); ?> <?php esc_html_e('says:','enspire'); ?></div>
