...
/* ------------------------------------ */
	private function _create_tabs($tabs,$count) {
		...
		$titles = array(
			'recent'	=> __('Recent Posts','enspire'),
			'popular'	=> __('Popular Posts','enspire'),
			'comments'	=> __('Recent Comments','enspire'),
			'tags'		=> __('Tags','enspire')
		);
		$icons = array(
			...
			<ul id="tab-comments" class="alx-tab group <?php if($instance['comments_avatars']) { echo 'avatars-enabled'; } ?>">
				...
				<li>
						...
						<div class="tab-item-inner group">
							...
							<div class="tab-item-name"><?php echo esc_attr( $comment->comment_author ); ?> <?php _e('says:','enspire'); ?></div>
